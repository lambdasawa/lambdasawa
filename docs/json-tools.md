...
- 概要を把握したいなら `jless` が良い
- 欲しい値が分かっててパスが分からないなら `gron` して `grep` にパイプすると良い
- その他の場合は基本的に `jiq` で対話的にクエリする
- CI やスクリプトで何度も非対話的にクエリする必要があるなら、 `jiq -q` でインタラクティブにクエリを考えてそれを `jq` に渡す
- `sed`, `awk` に自信がないか csv, xml, yaml, ini, .env をクエリしたいときは `jc` と上記のツールの組み合わせが活躍する
...
ここから下はある程度スター数があったりしてメモ程度に掲載しておく意義はあるが、別のツールで代替しているため普段は使っていないもの。

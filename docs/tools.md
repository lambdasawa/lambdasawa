...
- [λ沢を支える技術](#λ沢を支える技術)
  ...
  - [その他の CLI ツール](#その他の-cli-ツール)
    ...
    - [trans](#trans)
    - [glow](#glow)
    - [lambdasawa/dotfiles の bin](#lambdasawadotfiles-の-bin)
      - [tmuxopen](#tmuxopen)
...
指をいたわるために [alias](https://github.com/lambdasawa/dotfiles/blob/main/.config/fish/alias.fish) を活用していている。
...
いくつかサブコマンドを追加している。
`.gitconfig` の [alias](https://github.com/lambdasawa/dotfiles/blob/main/.config/git/alias) 由来のもの、 [git-extras](https://github.com/tj/git-extras/blob/master/Commands.md) 由来のもの、 `dotfiles` の [bin](https://github.com/lambdasawa/dotfiles/tree/main/bin) 由来のものがある。
...
$ glow README.md
  ...
  Image: shell.png → /Users/lambdasawa/dotfiles/assets/shell.png

  ## Setup

  ### Local

    git clone git@github.com:lambdasawa/dotfiles.git ~/dotfiles &&\
      cd ~/dotfiles &&\
      ./update
```

### [lambdasawa/dotfiles の bin](https://github.com/lambdasawa/dotfiles/tree/main/bin)

dotfiles で色々便利スクリプトを用意してる。いくつかよく使うもの or 多少複雑なものを選択してここに記載する。

#### [tmuxopen](https://github.com/lambdasawa/dotfiles/blob/dd59a9381674602eb02256329fa38e4c75d0a795/bin/tmuxopen)

ghq で管理されているリポジトリを fuzzy filter で選んで tmux で開くコマンド。

#### [wait-change](https://github.com/lambdasawa/dotfiles/blob/dd59a9381674602eb02256329fa38e4c75d0a795/bin/wait-change)
...
#### [ghs](https://github.com/lambdasawa/dotfiles/blob/08516f3cceec3388130a3fab19f17b6067a91eff/bin/ghs)

自分の GitHub/Gist を検索するコマンド。

`gists typescript` で自分が書いた TypeScript に関するコードと Gist 一覧がブラウザで開かれる。

#### [wait-gha](https://github.com/lambdasawa/dotfiles/blob/dd59a9381674602eb02256329fa38e4c75d0a795/bin/wait-gha)

GitHub Actions の終了を待つコマンド。

これも `wait-change` と同様にジョブが完了するとデスクトップ通知と音声通知を受け取ることができる。

#### [selectnpm](https://github.com/lambdasawa/dotfiles/blob/dd59a9381674602eb02256329fa38e4c75d0a795/bin/selectnpm)

カレントディレクトリのリポジトリにある npm モジュールを一覧で出して fuzzy filter で選択するコマンド。

npm のモノリポを扱っているときに `cd $(selectnpm)` を実行すると任意のサブパッケージにシュッと移動できて便利。

#### [cloudwatch-logs-query](https://github.com/lambdasawa/dotfiles/blob/e93bc076ea0208620f16cea2fe509bd9db38e7d3/bin/cloudwatch-logs-query)
